...
public class ReportCommand extends Command implements TabExecutor
{
    ...
    public void execute(final CommandSender sender, final String[] args) {
        ...
        requestWebHook("{\n  \"username\": \"%reporter%\",\n  \"avatar_url\": \"https://crafatar.com/avatars/%reporter_uuid%\",\n  \"content\": \""+ConfigManager.getReportMention()+"\",\n  \"embeds\": [\n    {\n      \"title\": \"\u9bd6\u540d\",\n      \"color\": 16711680,\n      \"description\": \"%servername%\",\n      \"timestamp\": \"\",\n      \"url\": \"\",\n      \"author\": {\n        \"name\": \"%reporter%\"\n      },\n      \"image\": {},\n      \"thumbnail\": {},\n      \"footer\": {},\n      \"fields\": [\n        {\n          \"name\": \"\u5bfe\u8c61\u8005\",\n          \"value\": \"%player%\",\n          \"inline\": false\n        },\n        {\n          \"name\": \"\u5185\u5bb9\",\n          \"value\": \"%content%\"\n        },\n        {\n          \"name\": \"UUID\",\n          \"value\": \"%reporter_uuid%\"\n        }\n      ]\n    }\n  ]\n}".replace("%servername%", ((ProxiedPlayer)sender).getServer().getInfo().getName()).replace("%player%", args[0]).replace("%player%", args[0]).replace("%uuid%", String.valueOf(ProxyServer.getInstance().getPlayer(args[0]).getUniqueId())).replace("%reporter%", sender.getName()).replace("%reporter_uuid%", String.valueOf(((ProxiedPlayer)sender).getUniqueId())).replace("%content%", String.join(" ", (CharSequence[])args).replace("\"", "\\\"")), ConfigManager.getReportURL());
    }
